...
import type { ChatMessage, ChatRole, SessionData } from "./redis";
import { formatLayerCatalogPrompt } from "./mapLayers";
import { openai } from "./openaiClient";
import {
  govmapTools,
  GovmapCommand,
  GOVMAP_API_GUIDE,
  handleGovmapToolCall
} from "./govmapTools";
import { executeServerToolCalls, serverTools } from "./serverTools";
...
function combineToolResultsByCallId(
  toolResults: any[]
): Array<{ tool_call_id: string; items: any[] }> {
...
chatRouter.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const rawUserMessage: unknown = req.body.message;
    const hasUserText =
      typeof rawUserMessage === "string" && rawUserMessage.trim().length > 0;
    const userMessageText: string | undefined = hasUserText
      ? (rawUserMessage as string)
      : undefined;
    ...
    if (hasToolResults) {
      ...
      const missingId = toolResults.filter(
        (r: any) => typeof r?.tool_call_id !== "string"
      );
    ...
    const layerContext = formatLayerCatalogPrompt(session.mapLayers);
    ...
    const { history, commands } = await runAiGovmapLoop(
      messagesForModel
    );
 ...
 */
async function runAiGovmapLoop(
  messages: ChatCompletionMessageParam[]
): Promise<{ history: ChatMessage[]; commands: GovmapCommand[] }> {
  const model = DEFAULT_MODEL;
  const history: ChatMessage[] = [];
  const commands: GovmapCommand[] = [];
  ...
  for (let i = 0; i < MAX_TOOL_LOOPS; i++) {
    const completion = await openai.chat.completions.create({
      ...
      tools: [...govmapTools, ...serverTools] as ChatCompletionTool[],
      tool_choice: "auto"
    });
    ...
    if (toolCalls.length > 0) {
      const govmapCalls = toolCalls.filter(
        (t) => t.type === "function" && t.function?.name === "govmap_call"
      );
      ...
      if (serverCalls.length > 0) {
        ...
        pushed = true;
        workingMessages.push(msg as ChatCompletionMessageParam);

        const toolMessages = await executeServerToolCalls(
          serverCalls as ChatCompletionMessageToolCall[]
        );
      ...
      if (govmapCalls.length > 0) {
        if (!pushed) history.push(assistantMessage);
        ...
        for (const call of govmapCalls) {
          ...
          const result = handleGovmapToolCall(args);
      ...
      if (serverCalls.length > 0) continue;
    }
...
function trimHistory(messages: ChatMessage[], limit = HISTORY_LIMIT): ChatMessage[] {
  if (messages.length <= limit) return messages;
  return messages.slice(messages.length - limit);
}

function mapMessagesForModel(messages: ChatMessage[]): ChatCompletionMessageParam[] {
...
function withCacheControl(msg: ChatCompletionMessageParam): ChatCompletionMessageParam {
  (msg as any).cache_control = CACHEABLE;
  return msg;
}
 ...
 */
async function summarizeHistory(session: SessionData, model: string): Promise<void> {
  if (session.messages.length <= SUMMARY_THRESHOLD) return;
