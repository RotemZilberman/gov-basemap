...
exports.govmapTools = exports.GOVMAP_API_GUIDE = void 0;
exports.handleGovmapToolCall = handleGovmapToolCall;
/**
...
- draw({ drawType: govmap.drawType.[Point|Polyline|Polygon|Rectangle|Circle] }): user draws geometry; callback returns WKT.
- editDrawing(): user edits last drawn geometry; callback returns WKT.
      ...
      `.trim(),
            parameters: {
                ...
                properties: {
                    commands: {
                        type: "array",
                        description: "A list of GovMap commands (fn + args) that the frontend will execute in order using govmap[fn](args).",
                        items: {
                            type: "object",
                            properties: {
                                fn: {
                                    type: "string",
                                    description: "GovMap JS function name, e.g. 'zoomToXY', 'setVisibleLayers', 'intersectFeatures', 'filterLayers', 'gpsOn', 'GeocodeString'. Use only functions from the GovMap API reference."
                                },
                                args: {
                                    type: "object",
                                    description: "Arguments object for that function, following the GovMap JS API patterns. Use the named keys shown in the reference above (e.g. 'x', 'y', 'level', 'layers', 'whereClause', 'returnFields').",
                    ...
                    explanation: {
                        type: "string",
                        description: "Short natural-language explanation (Hebrew or English, depending on the user's language) of what changed on the map or what the commands do. Example: 'מרכזתי את המפה על הכתובת ומיקדתי לשכבת המבנים.' or 'I centered the map on the address and zoomed to the buildings layer.'"
                    }
                },
                required: ["commands"]
            }
 ...
 */
function handleGovmapToolCall(args) {
    const commands = Array.isArray(args.commands)
        ? args.commands
            .filter((cmd) => cmd && typeof cmd.fn === "string")
            .map((cmd) => normalizeGovmapCommand(cmd))
        : [];
    return {
        commands,
...
function normalizeGovmapCommand(rawCmd) {
    const fn = rawCmd.fn;
    const args = typeof rawCmd.args === "object" && rawCmd.args !== null ? rawCmd.args : {};
    const toolCallId = typeof rawCmd.tool_call_id === "string" ? rawCmd.tool_call_id : undefined;
    return {
        fn,
...
function normalizeGovmapArgs(fn, rawArgs) {
    const args = typeof rawArgs === "object" && rawArgs !== null ? { ...rawArgs } : {};
    // Already using _UNPACK_ARGS_: normalize only.
    if (Array.isArray(args._UNPACK_ARGS_)) {
        return {
            _UNPACK_ARGS_: trimTrailingUndefined(args._UNPACK_ARGS_)
        };
    }
